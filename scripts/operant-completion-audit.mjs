...
const composeEvidenceInputs = [
  ...
  "apps/control-plane/migrations/008_audit_actor_slack_user_id.sql",
  "apps/control-plane/migrations/009_msteams_support.sql",
  "apps/control-plane/migrations/010_channel_policies_team_unique.sql",
  "apps/control-plane/migrations/011_admin_sessions_principal_platform.sql",
...
async function main() {
  ...
  await patternCheck("compose", "optional Docker sandbox overlay", "docker-compose.sandbox.yml", [
    ...
    ["channel plugin bootstrap", /operant-ensure-channel-plugins/],
  ]);
  await patternCheck("compose", "OpenClaw gateway image", "deploy/openclaw/Dockerfile.gateway", [
    ...
    ["channel plugin package dir", /mkdir -p \/usr\/local\/share\/operant\/openclaw\/plugins/],
    ["channel plugin bootstrap copy", /COPY deploy\/openclaw\/ensure-channel-plugins\.sh \/usr\/local\/bin\/operant-ensure-channel-plugins/],
    ...
    ["Teams plugin packed into image", /npm pack --pack-destination \/usr\/local\/share\/operant\/openclaw\/plugins @openclaw\/msteams@\$\{OPENCLAW_VERSION\}/],
  ...
  await patternCheck("compose", "OpenClaw channel plugin bootstrap", "deploy/openclaw/ensure-channel-plugins.sh", [
    ...
    ["local Teams package target", /ensure_plugin "msteams" "\/usr\/local\/share\/operant\/openclaw\/plugins\/openclaw-msteams-\*\.tgz"/],
    ["local Operant package target", /operant-openclaw-plugin-\*\.tgz/],
    ["Operant plugin version-aware upgrade", /openclaw plugins install --force "\$tarball"/],
    ["test harness prune", /test-harness/],
    ["failure is surfaced", /OpenClaw Slack plugin is not installed/],
    ["Teams failure is surfaced", /OpenClaw Microsoft Teams plugin is not installed/],
  ...
  await patternCheck("security", "admin login bootstrap secret", "apps/control-plane/src/server.ts", [
    ...
    ["header auth chat principal validation", /function actorChatPrincipal[\s\S]*x-operant-chat-platform[\s\S]*x-operant-principal-id[\s\S]*slackIdSchema\.safeParse/],
    ["first credential setup requires admin token", /handleCredentials[\s\S]*credentials\.bootstrap_denied/],
    ["first credential setup requires owner", /handleCredentials[\s\S]*owner_principal[\s\S]*adminSlackUserId[\s\S]*adminTeamsAadUserId[\s\S]*workspace owner/],
  ]);
  ...
  await patternCheck("security", "credential setup transactional write boundary", "apps/control-plane/src/server.ts", [
    ...
    ["credential setup retains admin approval approver", /const rawSlackApprovers[\s\S]*input\.approvalSlackUserIds[\s\S]*existingApprovalSlackUserIds[\s\S]*const approvalSlackUserIds = Array\.from\(new Set\(\[[\s\S]*input\.adminSlackUserId[\s\S]*rawSlackApprovers/],
  ...
  await patternCheck("openclaw", "OpenClaw Slack reuse config contract", "apps/control-plane/src/openclaw-config.ts", [
    ...
    ["Slack plugin allowlisted", /if \(slackConfigured\(input\)\) \{\s*allow\.push\("slack"\)/],
    ["Operant plugin entry registered", /entries\.operant\s*=\s*\{\s*enabled:\s*true/],
    ["Slack plugin enabled", /entries\.slack\s*=\s*\{\s*enabled:\s*true/],
