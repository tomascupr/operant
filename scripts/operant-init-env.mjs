...
const adminLoginToken = randomToken("operant_admin");
output = replaceEnvValue(output, "OPERANT_ADMIN_LOGIN_TOKEN", adminLoginToken);
...
process.stdout.write(`Wrote ${outputPath} (mode 0600)\n`);
process.stdout.write("\nNext steps:\n");
process.stdout.write("  1. pnpm compose:up -- -d\n");
process.stdout.write(`  2. open http://localhost:${httpPort}\n`);
process.stdout.write("  3. Sign in with your Slack user ID and this admin login token:\n");
process.stdout.write(`       ${adminLoginToken}\n`);
process.stdout.write(`     (also at OPERANT_ADMIN_LOGIN_TOKEN in ${outputPath})\n`);
process.stdout.write("\nThen save Slack and model credentials in the dashboard.\n");
